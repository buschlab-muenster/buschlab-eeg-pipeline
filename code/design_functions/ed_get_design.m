function D = ROSA2_get_design(designidx)
...
%  Copyright (C) 2019 Wanja Moessing & Svea Schroeder
%
...
% OLD DESIGN  FROM ROSA2
% D(1).factor_names  = {'target_cue_w', 'saccade_cue_w', 'response_correct'};
% D(1).factor_names_label = {'memcue', 'sacccue', 'response_correct'};
% % note: adding MemU and MemD will only add storage, it does not affect the
% % average (i.e., runtf won't add MemU twice to the last field in the cell).
% % Runtf will also not compute trials twice.
% D(1).factor_values = {{'MemR', 'MemL', @(x) ismember(x, {'MemU', 'MemD'}), 'MemU', 'MemD'},...
%     {'SaccR', 'SaccL', 'SaccC'}, {1 0}};
% D(1).factor_values_label = {{'right', 'left', 'neutral', 'up', 'down'},...
%     {'right', 'left', 'neutral'}, {'correct', 'wrong'}};
