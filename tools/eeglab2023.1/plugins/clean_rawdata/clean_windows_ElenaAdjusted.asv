...
for ii = 1:size(removed_samples,1)
        n = length(signal.event);
            ...
            signal.event(n+1).latency = removed_samples(ii,1);
            signal.event(n+1).duration = 0; % + half a second to compensate for the start extension and + 1 second to compensate for the end extension
            signal.event(n+1).type = 'auto_start';
        
            signal.event(n+2).latency = removed_samples(ii,end);
            signal.event(n+2).duration = 0;
            signal.event(n+2).type = 'auto_end';
end
